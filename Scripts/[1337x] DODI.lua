...
if version < 3.52 then
    ...
    local function request1337x()
        ...
        for i = 1, #rows do
            ...
            -- Check if this row contains a DODI upload
            -- Look for td with class "coll-5 vip" containing DODI user link
                ...
                if torrentLink and torrentName then
                    ...
                    if torrentPageHtml then
                        ...
                        if size ~= "" and seeds ~= "" and leeches ~= "" and uploadDate ~= "" then
                            tooltip = string.format("Size: %s | Seeds: %s | Leeches: %s | Upload date: %s | Uploader: DODI", 
                                size, seeds, leeches, uploadDate)
                        elseif size ~= "" and seeds ~= "" and uploadDate ~= "" then
                            tooltip = string.format("Size: %s | Seeds: %s | Upload date: %s | Uploader: DODI", 
                                size, seeds, uploadDate)
                        else
                            tooltip = "Uploader: DODI"
                        end
                        
                        local searchResult = {
                            name = "[" .. (size ~= "" and size or "Unknown") .. "] " .. torrentName,
                            links = {},
                            ScriptName = "[1337x] DODI",
                            tooltip = tooltip,
                            metadata = {
                                uploader = "DODI"
                            }
