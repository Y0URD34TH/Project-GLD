...
local VERSION = "1.0"
client.auto_script_update("https://raw.githubusercontent.com/Y0URD34TH/Project-GLD/refs/heads/main/Scripts/%5B1337x%5D%20Movies.lua", VERSION)
...
if version < 3.52 then
    ...
    Notifications.push_success("Lua Script", "[1337x] Movies script is loaded and working!")
...
local urlrequest = "https://" .. searchprovider .. "/category-search/" .. tostring(searchterm):gsub(" ", "+") .. "/Movies/1/"
local htmlContent = http.get(urlrequest, headers)
        ...
        if #results > 0 then
            ...
            Notifications.push("Search results", "No movies found.")
        end
    end
