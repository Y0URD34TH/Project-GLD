...
client.auto_script_update("https://raw.githubusercontent.com/Y0URD34TH/Project-GLD/refs/heads/main/Scripts/AnkerGames.lua", "1.3.0")
...
local function ondownloadcompleted(path, url)
	...
	if not path:lower():find("%.zip$") then
            Notifications.push_error("AnkerGames", "Downloaded file is not a ZIP archive. Extraction aborted.")
            return
	else
	    Notifications.push_warning("AnkerGames", "CMD will open for Extraction.")
        end
        path = path:gsub("\\", "/")
        pathcheck = path
        local downloadDir = path:match("(.*)/")
        local gamenametopath = gamename:gsub(":", "")
        local extractionDir = downloadDir .. "/" .. gamenametopath
        file.create_directory(extractionDir)
        local cmd = string.format('tar -xf "%s" -C "%s"', path, extractionDir)
        local output = system_output(cmd)
        onextractioncompleted(path, extractionDir)
    end
    settings.save()
end
