...
local VERSION = "1.0.0"
client.auto_script_update("https://raw.githubusercontent.com/Y0URD34TH/Project-GLD/refs/heads/main/Scripts/%5Bbackup%5D%20Atop-Games.lua", VERSION)
local sourcelink = "https://hydralinks.pages.dev/sources/atop-games.json"
...
if version < 6.95 then -- 3.50
    ...
    Notifications.push_success("Lua Script", "Atop Script Loaded and Working")
    menu.add_input_text("Atop Game Dir")
    menu.set_text("Atop Game Dir", defaultdir)
    settings.load()
    local function atopsearch()
        settings.save()
    ...
    client.add_callback("on_scriptselected", atopsearch)
